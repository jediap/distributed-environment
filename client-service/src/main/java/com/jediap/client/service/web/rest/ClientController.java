...
public class ClientController {
  ...
  public String clientService() {
    ...
    elasticsearchRepository.persistence(
        new Client(UUID.randomUUID().toString(), UUID.randomUUID().toString()), CLIENT_INDEX);

    elasticsearchRepository.persistence(
        new Client("941125e7-afae-486e-9fc7-57b01cc5bfa4", "jesus"), CLIENT_INDEX);

    elasticsearchRepository.delete("f9a0db32-0b3f-4134-a0cd-163a9e799e54", CLIENT_INDEX);

    List<Client> list =
        elasticsearchRepository.find(new HashMap<>(), PageRequest.of(0, 100), Client.class);
